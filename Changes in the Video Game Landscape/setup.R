...
# salesplot <- plot_ly(sales, x = ~Date, y = ~Sold, color = ~Hardware, type = 'scatter', mode = 'lines+markers',
#         marker = list(size = 5)) %>%
#   #add_trace(x = ~Date, y = ~Sold) %>%
#   add_annotations(x = x,
#                   y = y,
#                   text = labels) 
salesplot <-
  ...
  geom_point() +
  geom_line() +
  #geom_label(aes(label = Sold, size = NULL), nudge_y = 0.7) +
...
# salesplot <- salesp +
#   annotate("text", x = as.Date("2019-11-30", "%Y-%m-%d"),
#          y = 1920, label = "NS Peak")

salesplot <- ggplotly(salesplot, tooltip = c("Hardware", "Sold")) 
# %>%
